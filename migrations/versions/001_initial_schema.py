...
def upgrade() -> None:
    ...
    op.create_index('idx_speakers_name', 'speakers', ['name'], postgresql_using='gin', postgresql_ops={'name': 'gin_trgm_ops'})
    ...
    op.create_index('idx_legislation_title', 'legislation', ['title'], postgresql_using='gin', postgresql_ops={'title': 'gin_trgm_ops'})
    ...
    op.create_index('idx_vector_embeddings_sentence', 'vector_embeddings', ['sentence_index'])
    op.execute('CREATE INDEX idx_vector_embeddings_embedding ON vector_embeddings USING ivfflat (embedding vector_cosine_ops) WITH (lists = 100)')
